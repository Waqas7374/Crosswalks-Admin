<?php
	...
	switch ($boundary2) {
		case "Zip"://boundary1 as b1, zip as b2
			$sql = "Select distinct(b2.gid,b1.gid) as gid, " .$b1_id. " as b1_id, " .$b1_name. " as b1_name, b2.name as b2_id, concat('Zip ',b2.gid) as b2_name, (st_area(st_intersection(b1.geom, b2.geom))/st_area(b2.geom)*100) as perc_intersection, (st_area(st_intersection(b1.geom, b2.geom))/st_area(b1.geom)*100) as area_covered, s.stusps, ST_AsGeoJSON(b2.geom) AS geojson from ". $boundary1 ." as b1, zip as b2, states as s where st_intersects(b1.geom,b2.geom) AND NOT ST_Touches(b1.geom, b2.geom) and " .$filter. ";";
			echo $sql;
		break;
		...
		case "MSAs"://boundary1 as b1, msas as b2
			// $sql = "Select distinct(b2.gid,b1.gid) as gid, " .$b1_id. " as b1_id, " .$b1_name. " as b1_name, b2.geoid as b2_id, b2.name as b2_name, (st_area(st_intersection(b1.geom, b2.geom))/st_area(b2.geom)*100) as perc_intersection, (st_area(st_intersection(b1.geom, b2.geom))/st_area(b1.geom)*100) as area_covered, b2.stusps, ST_AsGeoJSON(b2.geom) AS geojson from ". $boundary1 ." as b1, msas as b2 where  st_intersects(b1.geom,b2.geom) AND NOT ST_Touches(b1.geom, b2.geom) and (".$filter.") and (st_area(st_intersection(b1.geom, b2.geom))/st_area(b1.geom)*100)>1;";
		...
		case "SubBasin": //boundary1 as b1, HUC8 as b2
			$sql = "Select distinct(b2.gid,b1.gid) as gid, " .$b1_id. " as b1_id, " .$b1_name. " as b1_name, concat('HUC-', b2.huc) as b2_id, b2.name as b2_name, (st_area(st_intersection(b1.geom, b2.geomsimple))/st_area(b2.geomsimple)*100) as perc_intersection, (st_area(st_intersection(b1.geom, b2.geomsimple))/st_area(b1.geom)*100) as area_covered, b2.states as stusps, ST_AsGeoJSON(b2.geomsimple) AS geojson from ". $boundary1 ." as b1, subbasin as b2 where st_intersects(b1.geom,b2.geomsimple) AND NOT ST_Touches(b1.geomsimple, b2.geomsimple) and ". $filter . ";";
		break;
